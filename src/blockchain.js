...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            b.time = new Date().getTime().toString().slice(0,-3);
            if(0<=height) {
                b.previousBlockHash = this.chain[this.height].hash;
            }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           var block = this.chain.filter(function(b) {
               return b.hash == hash;
           });
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for(let i= 1; i<this.chain.length; i++) {
                ...
                if(currentBlock.previousBlockHash != previousBlock.hash) {
                    errorLog.push(`error: index ${i} hash does not match`);
                }

                if(!currentBlock.validate()) {
                    errorLog.push(`error: index ${i} validate error`);
                }
            }
            resolve(errorLog);
        });
